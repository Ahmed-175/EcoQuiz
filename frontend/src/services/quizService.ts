...
// Update a quiz
export const updateQuiz = async (id: string, data: Partial<CreateQuizForm>): Promise<Quiz> => {
...
export const togglePublishQuiz = async (id: string, publish: boolean): Promise<Quiz> => {
    const response = await api.patch(`/quizzes/${id}/publish`, { is_published: publish });
    return response.data.data;
};
...
// Unlike a quiz
export const unlikeQuiz = async (id: string): Promise<ApiResponse<{ likes_count: number }>> => {
    const response = await api.delete(`/quizzes/${id}/like`);
    return response.data;
};

// Check if user liked quiz
export const checkQuizLike = async (id: string): Promise<{ liked: boolean }> => {
...
// Start quiz attempt
export const startQuizAttempt = async (quizId: string): Promise<{ attempt_id: string; started_at: string }> => {
...
export const getUserQuizAttempts = async (quizId: string): Promise<QuizAttempt[]> => {
    const response = await api.get(`/quizzes/${quizId}/attempts`);
    return response.data.data;
};

// Get quiz leaderboard
export const getQuizLeaderboard = async (quizId: string, limit?: number): Promise<LeaderboardEntry[]> => {
    const response = await api.get(`/quizzes/${quizId}/leaderboard`, { params: { limit } });
    return response.data.data;
};
...
export const getUserCreatedQuizzes = async (userId: string): Promise<Quiz[]> => {
    const response = await api.get(`/users/${userId}/quizzes`);
    return response.data.data;
};

// Get community quizzes
export const getCommunityQuizzes = async (communityId: string): Promise<Quiz[]> => {
